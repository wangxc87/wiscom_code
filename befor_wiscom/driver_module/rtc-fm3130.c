...
bad_clock:
	...
	if (IS_ERR(fm3130->rtc)) {
		...
		printk("unable to register the class device\n");
		goto exit_free;
	}
    printk("register the class device OK\n");
	return 0;
exit_free:
	kfree(fm3130);
	return err;
...
{
        int ret;
        ret = i2c_add_driver(&fm3130_driver);
