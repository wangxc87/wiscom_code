...
int main(int argc, char *argp[])
{
  ...
  */
  ...
  /*
  if(resizer_config(rsz_fd,user_mode,&rsz_ss_config,0)<= -1)
  {
  ...
  */
  ...
  if(preview_config(prev_fd,user_mode,&prev_ss_config,0)<=-1)
  {
  ...
  */
  buf_out1.size = output1_size;
  buf_in.size = prev_bufin_size;
  ...
  /*  display_param.width  = out1_width;
  display_param.height = out1_height;
  */
  ...
  printf("\n----Device Init Over...\n");
  int frame_count= 0;
  unsigned long prev_time,cur_time,period_time;
  float framerate,timeperframe;
  ...
  try2:
    ...
    //cat_buf.m.userptr = input_buffer;
	...
	convert.in_buff.offset = (unsigned int)cap_buf.m.userptr;//buf_in.offset;
	convert.in_buff.size = buf_in.size;
	convert.out_buff1.buf_type = IMP_BUF_OUT1;
	convert.out_buff1.index = -1;
	convert.out_buff1.offset =(unsigned int)disp_buf.m.userptr;//buf_out1.offset;
    ...
    //printf("Error:Display MEMCPY...\n");
        //        disp_buf.m.userptr = cap_buf.m.userptr;

    cur_time = cap_buf.timestamp.tv_sec*1000000 + cap_buf.timestamp.tv_usec;
    ...
    if(0 == (frame_count%30))
    {
      period_time = cur_time - prev_time;
       framerate = 30.00*1000000/period_time;
       timeperframe = period_time/(30.0*1000);
      
      prev_time = cur_time;

      printf("Picture%d:framrate = %f,timeperframe = %f ms\n",frame_count,framerate,timeperframe);
