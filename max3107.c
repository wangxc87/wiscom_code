...
static const struct baud_table brg3p68_ext[] = {
	{ 300,    MAX3107_BRG3P68_B300 },
	{ 600,    MAX3107_BRG3P68_B600 },
	{ 1200,   MAX3107_BRG3P68_B1200 },
	{ 2400,   MAX3107_BRG3P68_B2400 },
	{ 4800,   MAX3107_BRG3P68_B4800 },
	{ 9600,   MAX3107_BRG3P68_B9600 },
	{ 19200,  MAX3107_BRG3P68_B19200 },
	{ 57600,  MAX3107_BRG3P68_B57600 },
	{ 115200, MAX3107_BRG3P68_B115200 },
	{ 230400, MAX3107_BRG3P68_B230400 },
	{ 460800, MAX3107_BRG3P68_B460800 },
	{ 921600, MAX3107_BRG3P68_B921600 },
	{ 0, 0 }
};
...
static int max3107_ctrl(struct max3107_port *s, u8 *tx, u8 *rx, int len)
{
	struct spi_message spi_msg;
	struct spi_transfer spi_xfer;
    int i = 0;
    u16 *prx,*ptx, etx,erx,tmp;
    printk("%s enter...\n",__func__);
    ptx = (u16 *)tx;
    if(rx)
        prx = (u16 *)rx;
    ...
    for(i = 0; i < len/2; i++)
    {
        ...
        /* spi_xfer.len = 2; */
        /* spi_xfer.tx_buf = (u8 *)&etx; */
        /* spi_xfer.rx_buf = (u8 *)&erx; */
...
#else
/* Perform SPI transfer for write/read of device register(s) */
int max3107_ctrl(struct max3107_port *s, u8 *tx, u8 *rx, int len)
{
	struct spi_message spi_msg;
	struct spi_transfer spi_xfer;
a
	...
	spi_xfer.len = len;
#ifdef CONFIG_WISCOM1
	spi_xfer.tx_buf = (u8 *)etx;//tx;
	spi_xfer.rx_buf = (u8 *)erx;//rx;
#else
	spi_xfer.tx_buf = tx;
	spi_xfer.rx_buf = rx;
#endif
...
EXPORT_SYMBOL_GPL(max3107_ctrl);
#endif
...
static void max3107_handlerx(struct max3107_port *s, u16 rxlvl)
{
	...
	while (rxlvl) {
		...
		if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, len * 2)) {
        ...
        pr_debug("%s: recv data is %s, len-%d.\n",__func__, (u8 *)buf, rxlvl);
	...
	if (s->rx_enabled) {
		...
		pr_debug("%s: Enabling RX INT\n", __func__);
		buf[0] = (MAX3107_WRITE_BIT | MAX3107_IRQEN_REG);
		s->irqen_reg |= MAX3107_IRQ_RXFIFO_BIT;
		buf[0] |= s->irqen_reg;
		if (max3107_ctrl(s, (u8 *)buf, NULL, 2))
...
static void max3107_handletx(struct max3107_port *s)
{
	...
	if (!s->tx_fifo_empty){
        ...
        pr_debug("%s: tx_fifo not empty,return.\n", __func__);
		return;
    }

	if (uart_circ_empty(xmit) || uart_tx_stopped(&s->port)){
		/* No data to send or TX is stopped */
        pr_debug("%s: circ_empty-%d, tx_stopped-%d, return.\n", __func__, uart_circ_empty(xmit),uart_tx_stopped(&s->port));
		return;
    }
	...
	if (len) {
		...
		/* Perform the SPI transfer */
		if (max3107_ctrl(s, (u8 *)buf, NULL, len*2)) {
			dev_err(&s->spi->dev,
...
{
	u16 buf[8];	/* Buffer for SPI transfers */
	u8 irq_status;
	u16 rx_level;
	unsigned long flags;
    ...
    buf[4] = MAX3107_SPCHR_IRQSTS_REG;
	if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, 8)) {
		dev_err(&s->spi->dev,
	...
	irq_status = (u8)buf[0];
	pr_debug("%s: IRQSTS-0x%x \n",__func__, irq_status);
	pr_debug("%s: IRQSTS-0x%x, STS_IRQSTS-0x%x, LSR_IRQSTS-0x%x, rxfifolvl-%d, spchr_irqsts-0x%x\n",
	...
	if (irq_status & MAX3107_IRQ_TXEMPTY_BIT) {
		 ...
		 */
		pr_debug("%s: TE INT, disabling TX\n", __func__);
...
static void max3107_dowork(struct max3107_port *s)
{
	if (!work_pending(&s->work) && !freezing(current) && !s->suspended)
		...
		dev_warn(&s->spi->dev, "interrup isn't serviced normally, pending-%d,freezing-%d, suspend-%d!\n",
                 work_pending(&s->work), freezing(current), s->suspended);
...
static int max3107_read_id(struct max3107_port *s)//wiscom
{
	...
	if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, 2*i)) {
...
static void max3107_work(struct work_struct *w)
{
	...
	if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, 2)) {
...
static void max3107_register_init(struct max3107_port *s)
{
	...
	if (s->ext_clk) {
		s->brg_cfg = MAX3107_BRG3P68_B9600;
		s->baud_tbl = (struct baud_table *)brg3p68_ext;//brg26_ext; wiscom
	} else {
	...
	/* buf[8] |= (MAX3107_FLOWCTRL_AUTOCTS_BIT | MAX3107_FLOWCTRL_AUTORTS_BIT);wiscom */
	...
	/* buf[10] |= MAX3107_LSR_RXTO_BIT; wiscom*/

	/* Perform SPI transfer */
	if (max3107_ctrl(s, (u8 *)buf, NULL, 22))
	...
	s->irqen_reg |= MAX3107_IRQ_RXFIFO_BIT | MAX3107_IRQ_TXEMPTY_BIT ;
...
#ifdef CONFIG_WISCOM1
buf[3] = (MAX3107_WRITE_BIT | MAX3107_LSR_IRQEN_REG);
buf[4] = (MAX3107_WRITE_BIT | MAX3107_STS_IRQEN_REG);
buf[5] = (MAX3107_WRITE_BIT | MAX3107_SPCHR_IRQEN_REG);
#endif
	/* Perform SPI transfer */
	if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, 6))
		dev_err(&s->spi->dev, "SPI transfer for init failed\n");
...
int max3107_probe(struct spi_device *spi, struct max3107_plat *pdata)
{
	...
	spi->bits_per_word = 8; //default wiscom
	s->ext_clk = pdata->ext_clk;
	s->loopback = pdata->loopback;
	spi_setup(spi);
	s->spi = spi;
#ifdef CONFIG_WISCOM
	buf[0] = (MAX3107_WRITE_BIT | MAX3107_MODE2_REG);
	/* Reset FIFOs */
	buf[0] |= MAX3107_MODE2_RST_BIT;
	if (max3107_ctrl(s, (u8 *)buf, (u8 *)buf, 2)) {
		dev_err(&s->spi->dev, "SPI transfer for RST failed\n");
		retval = -EIO;
		goto err_free1;
	}
    msleep(200);    
	...
	/* Perform SPI transfer */
	/* if (max3107_ctrl(s, (u8 *)buf, NULL, 4)) {//default 4 */
	/* 	dev_err(&s->spi->dev, "SPI transfer for init failed\n"); */
	/* 	retval = -EIO; */
	/* 	goto err_free1; */
	/* } */
