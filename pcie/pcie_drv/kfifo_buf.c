...
int OSA_queDelete(struct kfifo_que_obj *kque_hndl)
{
...
int OSA_queCreate(struct kfifo_que_obj *kque_hndl, int nums)
{
    int ret;
    struct kfifo *kfifo_hndl = &kque_hndl->kfifo_hndl;
...
#ifdef CONFIG_WAITQUE_USED
    init_waitqueue_head(&kque_hndl->kfifo_que_wq_h);
#endif
    return 0;
}

int OSA_quePut(struct kfifo_que_obj *kque_hndl, int value, int timeout)
{
...
int OSA_queGet(struct kfifo_que_obj *kque_hndl, int *value, int timeout)
{
    char data;
    struct kfifo *kfifo_hndl = &kque_hndl->kfifo_hndl;
    if(!value){
...
int OSA_queReset(struct kfifo_que_obj *kque_hndl)
{
    kfifo_reset(&kque_hndl->kfifo_hndl);
    return 0;
}
//empty return True, otherwise FALSE
int OSA_queIsEmpty(struct kfifo_que_obj *kque_hndl)
{
    return kfifo_is_empty(&kque_hndl->kfifo_hndl);
}

//full return True, otherwise FALSE
int OSA_queIsFull(struct kfifo_que_obj *kque_hndl)
{
    return kfifo_is_full(&kque_hndl->kfifo_hndl);    
}
...
int OSA_bufReset(OSA_BufHndl *hndl)
{
    int i;
    OSA_queReset(&hndl->emptyQue);
    OSA_queReset(&hndl->fullQue);
