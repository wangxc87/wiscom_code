...
int pcie_buf_init(struct pcie_bufHndl *pBufHndl, struct kfifo_buf_create_arg *buf_create)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR  "%s: osa_bufCreate failed.\n", __FUNCTION__);
        goto err_exit1;
    }
...
char *pcie_buf_getEmtpy(struct pcie_bufHndl *pBufHndl)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR  "%s: OSA_bufGetEmpty [chnID-%d] failed.\n", __FUNCTION__, bufId);
        buf_unlock(pBufHndl);
        return NULL;
    }
    ...
    if (pBufinfo == NULL) {
        printk(KERN_ERR  "%s: OSA_bufGetBufInfo [bufId: %d] failed.\n", __FUNCTION__, bufId);
        buf_unlock(pBufHndl);
        return NULL;
    }
...
int pcie_buf_setEmpty(struct pcie_bufHndl *pBufHndl, char *buf_ptr)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR  "%s: OSA_bufPutEmpty [bufId: %d] failed.\n", __FUNCTION__, i);
        buf_unlock(pBufHndl);
        return -1;
    }            
    buf_unlock(pBufHndl);
...
int pcie_buf_getFull(struct pcie_bufHndl *pBufHndl, char **buf_ptr)
{
    ...
    if (ret < 0) {
        printk(KERN_ERR  "%s: OSA_bufGetEmpty failed.\n", __FUNCTION__);
        buf_unlock(pBufHndl);
        return -1;
    }
...
int pcie_buf_reset(struct pcie_bufHndl *pBufHndl)
{
    int ret = 0;

    buf_lock(pBufHndl);

    ret = OSA_bufReset(&pBufHndl->buf_handle);
