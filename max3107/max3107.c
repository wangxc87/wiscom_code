...
int max3107_rw(struct max3107_port *s, u8 *tx, u8 *rx, int len)
{
    ...
    do{
        ...
        memset(spi_xfer, 0, sizeof( struct spi_transfer)*count);
            
        for(i = 0; i < count; i++){
 
            /* Initialize SPI transfer */
            spi_xfer[i].cs_change = 1;
            spi_xfer[i].len = 2;
            spi_xfer[i].tx_buf = (u8 *)ptx;
            spi_xfer[i].rx_buf = (u8 *)prx;
            spi_xfer[i].speed_hz = MAX3107_SPI_SPEED;

            /* Add SPI transfer to SPI message */
            spi_message_add_tail(&spi_xfer[i], &spi_msg);
...
static void max3107_register_init(struct max3107_port *s)
{
    ...
    pr_debug("%s: Test spi, Read 8 devId: ", __func__);
    for(j=0; j<i; j++)
        pr_debug("0x%x  ", buf[j]);
    pr_debug("\n");
