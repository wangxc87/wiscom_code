...
void print_usage(char *program_name)
{
    fprintf(stderr,
            ...
            "*************************************\n\n"
            "Usage:\n  %s <device> <baud> <databits> <parity> <stopbits> <read_or_write> [-s send_strings -t send_interval]]\n"
            "       databits: 5, 6, 7, 8\n"
            "       parity: 0(None), 1(Odd), 2(Even)\n"
            "       stopbits: 1, 2\n"
            "       read_or_write: 0(read), 1(write)\n"
            "          -s        : send strings\n"
            "          -t        : send inteval (unit: us/ms/s)\n"
            "Example:\n  %s /dev/ttyS0 9600 8 0 1 0 -s abcd -t 10ms\n\n",
            program_name, program_name
            );
}

static void wait(unsigned int sec, unsigned int usec)
{
    struct timeval tv;
    tv.tv_sec = sec;
    tv.tv_usec = usec;
...
int main(int argc, char *argv[])
{
    ...
    int parity;
    int read_or_write;
    unsigned int usec = 0, sec = 0;
    char *optstring = "s:t:h";
    int ret,opt;
    char *p, char_buf[32];
    char dev_name[64];
    ...
    while(1){
        ...
        case 't':
            p = strstr(optarg, "us");
            if(p != NULL){
                strncpy(char_buf, optarg, strlen(optarg) -2);
                usec = atoi(char_buf);                
            }else if(strstr(optarg, "ms") != NULL){
                strncpy(char_buf, optarg, strlen(optarg) -2);
                usec = atoi(char_buf) * 1000;                
            }else if(strstr(optarg, "s") != NULL){
                strncpy(char_buf, optarg, strlen(optarg) -1);
                sec = atoi(char_buf);                
            }
            break;
        case 'h':
    ...
    {
        if(( 0 == usec) && (0 == sec))
            fprintf(stdout, "Begin to send:\n");
        else
            fprintf(stdout, "Begin to send \"%s\" interval %u sec-%u usec\n",buf, sec, usec);
        /*
