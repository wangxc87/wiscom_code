...
/* #define PCIE_INVALID_ID  (0) */
 ...
 /*         Name:  OSA_pcieInit */
 /*  Description:  pcie初始化 */
 /*  Input Args :  config初始化参数，为NULL，采用默认配置 */
 /*                config->en_clearBuf：TRUE清除接收缓冲（建议）, FALSE 不清除接收缓冲区 */
 /*                config->tv：初始化等待时间 */
 /*  Output Args:  无 */
...
/*                sync_mode:    发送数据模式， DATA_SEND_SYNC_MODE：确保数据被接收方正确接收  DATA_SEND_NOSYNC_MODE：数据可能被接收方丢弃 */
